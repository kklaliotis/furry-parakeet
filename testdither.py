...
for line in content:
    ...
    if m:
        sigout = numpy.sqrt(sigout**2 + float(m.group(1)) ** 2)
    ...
    if m:
        ...
        for j in range(len(roll)):
            roll[j] = float(roll[j]) * numpy.pi / 180.0  # convert to float
...
posoffset = []
if roll is None:
    roll = numpy.zeros((n_in,)).tolist()
if shear is None:
    shear = numpy.zeros((2 * n_in,)).tolist()
if magnify is None:
    magnify = numpy.zeros((n_in,)).tolist()
for k in range(n_in):
    fk = 0
    if messyPSF:
        fk = k
...
print("roll:", numpy.array(roll) * 180.0 / numpy.pi)
...
q2 = 2 * q1 + 1
hdu = fits.PrimaryHDU(
    numpy.transpose(P.psf_array[:, n1 - q1 : n1 + q1 + 1, n1 - q1 : n1 + q1 + 1], axes=(1, 0, 2)).reshape(
        q2, q2 * (n_in + n_out)
    )
)
hdu.writeto(outstem + "testpsf1b.fits", overwrite=True)
hdu = fits.PrimaryHDU(
    numpy.transpose(P.overlaparray, axes=(0, 2, 1, 3)).reshape(P.nsample * P.n_tot, P.nsample * P.n_tot)
)
...
for ipsf in range(n_out):
    print(
        f"error {ipsf:2d} {numpy.sqrt(numpy.sum(outerr[ipsf,:,:]**2)/numpy.sum(outtest[ipsf,:,:]**2)):11.5E}"
    )
    amp[ipsf] = numpy.sqrt(numpy.sum(outtest[ipsf, :, :] ** 2))

# test input with Gaussian white noise
noise_in = rng.normal(0.0, 1.0, size=(n_in * ny_in * nx_in,))
noise_out = (ims["T"].reshape((n_out, ny_out * nx_out, n_in * ny_in * nx_in)) @ noise_in).reshape(
    (n_out, ny_out, nx_out)
)
hdu = fits.PrimaryHDU(numpy.transpose(noise_out, axes=(1, 0, 2)).reshape(ny_out, n_out * nx_out))
hdu.writeto(outstem + "samplenoise.fits", overwrite=True)
...
for _ in range(1000):
    test_srcpos = (
        (s_out * (nx_out - 1) * 0.5 + extbdy) * (rng.random() - 0.5),
        (s_out * (ny_out - 1) * 0.5 + extbdy) * (rng.random() - 0.5),
    )
    ...
    # for ipsf in range(n_out):
    #   print('error {:3d},{:2d} {:11.5E}    :'.format(
    #   itest, ipsf, numpy.sqrt(numpy.sum(outerr[ipsf,:,:]**2))/amp[ipsf]))
    allerr += [numpy.sqrt(numpy.sum(outerr[ipsf, :, :] ** 2)) / amp[ipsf]]
allerr = numpy.array(allerr)
print(f"rms err = {numpy.sqrt(numpy.mean(allerr**2)):11.5E}")
print(f"med err = {numpy.median(allerr):11.5E}")
print(f"max err = {numpy.amax(allerr):11.5E}")

# print summary information
print("percentiles of sqrtUC, sqrtSigma, kappa:")
for i in [1, 2, 5, 10, 25, 50, 75, 90, 95, 98, 99]:
    print(
        "    {:2d}% {:11.5E} {:11.5E} {:11.5E}".format(
            i,
            numpy.percentile(numpy.sqrt(ims["UC"]), i),
            numpy.percentile(numpy.sqrt(ims["Sigma"]), i),
